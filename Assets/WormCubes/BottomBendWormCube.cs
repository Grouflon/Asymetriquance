...
public class BottomBendWormCube : WormCube
{
    ...
    public override void CalculateVertices(float _size, float _period, float _amplitude, float _t, out Vector3[] _vertices)
    {
        ...
        Quaternion rotation = Quaternion.AngleAxis(angle, _vertices[3] - _vertices[0]);
        
        _vertices[4] = _vertices[0] + rotation * new Vector3(_size, 0.0f, 0.0f);
        _vertices[7] = _vertices[3] + rotation * new Vector3(_size, 0.0f, 0.0f);

        _vertices[5] = _vertices[4] + rotation * rotation * new Vector3(0.0f, -_size, 0.0f);
        _vertices[6] = _vertices[7] + rotation * rotation * new Vector3(0.0f, -_size, 0.0f);
    }
